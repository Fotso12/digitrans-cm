...
public class BlockchainService {
    ...
    /**
     * Enregistrer un mouvement de stock sur la blockchain (Smart Contract TraçabilitéSupplyChain).
     * En cas de nœud blockchain inaccessible, retourne un hash simulé robuste pour éviter tout blocage.
     */
    public String enregistrerMouvement(
        ...
        try {
            // Simulation/Appel via Web3j du smart contract déployé à l'adresse configurée
            log.info("Appel de la fonction mettreAJourStatut() du smart contract à l'adresse : {}", contractAddress);
            
            // Dans une version de production, nous utiliserions le wrapper généré par Web3j CLI :
            // TraçabilitéSupplyChain contract = TraçabilitéSupplyChain.load(contractAddress, web3j, credentials, new DefaultGasProvider());
            // TransactionReceipt receipt = contract.mettreAJourStatut(codeMarchandise, nouveauStatut, localisation, remarques).send();
